...
class ZendExtensionTest extends TestCase
{
    ...
    public function testI18nLoad()
    {
        ...
        $configuration = $loader->loggerLoad(array(), $configuration);
        $this->assertEquals('Zend\Translator\Translator', $configuration->getParameter('zend.translator.class'), '->i18nLoad() loads the i18n.xml file if not already loaded');

        $configuration = $loader->loggerLoad(array('locale' => 'fr'), $configuration);
        $this->assertEquals('fr', $configuration->getParameter('zend.translator.locale'), '->i18nLoad() overrides existing configuration options');
    }
}
